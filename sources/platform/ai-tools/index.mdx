...
AI agents reach Apify through a live connection over the [MCP server](/integrations/mcp), a one-step [plugin](/ai-tools/plugin) for supported clients, [Agent Skills](/ai-tools/skills) that carry procedural knowledge, and [machine-readable documentation](/ai-tools/docs-for-agents). These stack rather than compete, and most setups use more than one.

If you want the shortest path from zero to a first Actor run, start with the [agent quickstart](/get-started/agent-onboarding). This page explains which pieces exist and when each one is the right choice.
...
The distinction that matters most: the MCP server gives an agent *capability*, and Agent Skills give it *procedure*. An agent with MCP alone can call any tool but has to work out the sequence itself. An agent with skills follows a workflow Apify already validated. Skills are not a separate integration method; they layer on top of a connection you already have.

## Choose a path

- If your coding agent has a [plugin](/ai-tools/plugin), install that. It configures MCP and skills together, so there's nothing else to wire up.
