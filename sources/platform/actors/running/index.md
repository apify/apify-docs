...
Internally, the `call()` function invokes the [Run Actor](/api/v2/actor-runs-post) API endpoint, waits for the Actor to finish, and reads its output using the [Get dataset items](/api/v2/dataset-items-get) API endpoint.
