...
1. In **Title**, add the status message. For example, _A new version coming soon_.
1. _(Optional)_ In **Description**, add an explanation of the status in a form of a tooltip.
1. In **Type**, choose the styling of the status:
    - **Info** (gray), suitable for general notes.
    - **Success** (green), suitable for positive updates. For example, an announcement of a new feature.
    - **Warning** (yellow), suitable for highlighting temporary issues. For example, an add-on not working.
    - **Danger** (red), suitable for critical issues. For example, the Actor experiencing downtime.
1. Select **Save**.
