...
The rental pricing model is being discontinued. Starting April 1, 2025, no new rentals are allowed, including edits to existing rentals. On October 1, 2025, all remaining rental Actors in Apify Store will be migrated to pay-per-usage pricing. We recommend switching to [pay-per-result](/platform/actors/publishing/monetize/pay-per-result) or [pay-per-event](/platform/actors/publishing/monetize/pay-per-event) pricing models.
