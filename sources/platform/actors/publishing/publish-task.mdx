...
The [Apify MCP server](/mcp) exposes task publishing as tools, so you can describe what you want in your AI client and let the agent do the work: `create-actor-task`, `get-actor-task`, `update-actor-task`, `publish-actor-task`, and `unpublish-actor-task`.
