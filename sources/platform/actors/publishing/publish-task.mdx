...
Publishing takes three steps, because creating a task and configuring its page never publish it on their own:

1. Create the task with its input.
1. Set the task's `title` and `description`, and its public display configuration (`publicConfig`).
1. Publish by setting `isPublic` to `true`.
...
- `publicConfig.datasetView` is one of the view keys declared in the Actor's [dataset schema](/storage/dataset-schema).
...
The [Apify CLI](/cli) publishes and unpublishes an existing task by name:
...
There is no dedicated command for the display configuration yet, so set it through the API with `apify api`. Prefix the task name with `~` so the API resolves it against your own tasks rather than reading it as a task ID:

```bash
apify api PUT actor-tasks/~my-task -d '{
    "publicConfig": {
