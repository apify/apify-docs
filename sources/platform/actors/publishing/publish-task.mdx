...
1. Complete the three required sections: [Page details](#page-details), [Input](#input), and [Dataset schema](#dataset-schema). Optionally, add an [FAQ](#faq).
...
The optional **FAQ** section adds questions and answers to the bottom of the landing page. An FAQ answers what people want to know before they run your task, and gives search engines and AI agents short facts they can quote.

- **Generate FAQ:** writes questions and answers with AI, based on the task's title, description, and input, and on the Actor's README and dataset schema. If the task already has an FAQ, the AI uses it as a draft and replaces it after you confirm.
- **Add question:** adds a question and answer that you write yourself.

Review and edit generated items before you save them. An FAQ holds up to 10 items. Each question can be up to 120 characters and each answer up to 250 characters.

![FAQ section with the Generate FAQ button, two question-and-answer pairs with character counters, and the Add question button.](./images/public-tasks/task-publishing-faq.webp)

:::tip Write answers that stand alone

AI agents and search engines often quote a single answer without its question. Name the task in each answer instead of starting with "It" or "Yes": "The Amazon price tracker task returns the product title, price, and image URL" rather than "Yes, it returns prices." Aim for 5 to 8 questions about what the output contains, what input the task needs, and how to run it through the API or on a schedule.
...
1. Publish it by setting `isPublic` to `true`, or by calling `publish()` in the API clients.
...
`publicConfig.faq` is optional too. It holds up to 10 `{ "question": "...", "answer": "..." }` objects, with questions up to 120 characters and answers up to 250 characters. Neither can be blank.
...
Create the task with [Create task API endpoint](/api/v2/actor-tasks-post), then configure and publish it with [Update task API endpoint](/api/v2/actor-task-put):
...
The landing page displays the information you configured in the [Page details](#page-details), [Input](#input), [Dataset schema](#dataset-schema), and [FAQ](#faq) sections:
...
- The [FAQ](#faq), if you added one. The markdown version of the page includes it too.
