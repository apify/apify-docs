...
description: Learn how the Apify QA system tests Actors, what causes an Actor to be flagged as under maintenance or deprecated, and how to restore a healthy status.
...
The test runs the Actor with its default input (defined by the [**prefill**](https://docs.apify.com/platform/actors/development/input-output-schemas/input-schema/specification/v1#prefill-vs-default-vs-required) option in the input schema file)
...
by using the public [Actor Testing](https://apify.com/pocesar/actor-testing) tool available in Apify Store.
