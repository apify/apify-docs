...
| `dockerfile` | Optional | The path to the Dockerfile to be used for building the Actor on the platform. If not specified, the system will search for Dockerfiles in the `.actor/Dockerfile` and `Dockerfiles` paths, in that order. Refer to the [Dockerfile](./dockerfile.md) section for more information. |
