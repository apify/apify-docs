...
1. Select the dataset you want to export.
1. Select **Export** and choose a format: JSON, CSV, XML, Excel, HTML, RSS, or JSONL.

:::caution Run page Export button

The **Export** button on the Run page exports only the `default` dataset. To export other datasets, use the **Storage** tab as described above.

:::

To export programmatically, call the [Dataset API](/api/v2/dataset-items-get) with the dataset ID from `ACTOR_STORAGES_JSON`, or use the SDK methods shown in [Access datasets in Actor code](#access-datasets-in-actor-code). See [Datasets](../../../../storage/dataset.md) for export formats and query parameters.
