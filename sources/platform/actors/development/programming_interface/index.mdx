---
title: Actor programming interface
sidebar_label: Programming interface
sidebar_position: 4
description: Learn about the programming interface of Apify Actors, important commands and features provided by the Apify SDK, and how to use them in your Actors.
slug: /actors/development/programming-interface
---

import Card from '@site/src/components/Card';
import CardGrid from '@site/src/components/CardGrid';

This chapter will guide you through all the commands you need to build your first Actor. This interface is provided by [Apify SDKs](/sdk). The chapter starts with basic commands and guides you through system events and environment variables that are available to your Actor both locally and when running on the Apify platform.

<CardGrid>
    <Card
        title="Basic commands"
        desc="Learn how to provide your Actor with context that determines its behavior through a plethora of pre-defined environment variables offered by the Apify SDK."
        to="/actors/development/programming-interface/basic-commands"
    />
    <Card
        title="Environment variables"
        desc="Learn how to provide your Actor with context that determines its behavior through a plethora of pre-defined environment variables offered by the Apify SDK."
        to="/actors/development/programming-interface/environment-variables"
    />
    <Card
        title="Status messages"
        desc="Use custom status messages to inform the user about the progress of your Actor run."
        to="/actors/development/programming-interface/status-messages"
    />
    <Card
        title="System events"
        desc="Learn about system events sent to your Actor and how to benefit from them."
        to="/actors/development/programming-interface/system-events"
    />
    <Card
        title="Container web server"
        desc="Learn how to run a web server inside your Actor, which enables you to communicate with the outer world via UI and API."
        to="/actors/development/programming-interface/container-web-server"
    />
    <Card
        title="Metamorph"
        desc="The metamorph operation transforms an Actor run into the run of another Actor with a new input."
        to="/actors/development/programming-interface/metamorph"
    />
    <Card
        title="Actor Standby"
        desc="Learn how to use Actors in lightweight mode as a blazingly fast API server."
        to="/actors/development/programming-interface/standby"
    />
    <Card
        title="Persisting state across runs"
        desc="Read data written by a previous run of the same Actor - useful for scheduled or cron-driven Actors."
        to="/platform/actors/development/programming-interface/persisting-state-across-runs"
    />
</CardGrid>
