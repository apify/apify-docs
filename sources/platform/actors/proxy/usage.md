...
To set a new session, pass the `session` parameter in your [username](./usage.md#username-parameters) field when connecting to a proxy. This will serve as the session's ID and an IP address will be assigned to it. To [use that IP address in other requests](/actors/proxy/datacenter-proxy#connecting-to-datacenter-proxies), pass that same session ID in the username field.
