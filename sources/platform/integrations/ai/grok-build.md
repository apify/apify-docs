...
The Apify plugin connects Grok Build to Apify's library of [Actors](https://apify.com/store) and bundles the [Apify MCP server](/integrations/mcp) for searching the Store, running Actors, and retrieving datasets through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/docs/getting-started/intro).
...
1. If the official marketplace (`plugin-marketplace`, `https://github.com/xai-org/plugin-marketplace.git`) isn't listed, add it:
...
1. Run `/plugins` to open the extensions manager, then open the **Marketplace** tab. The official `plugin-marketplace` appears in the list.

1. Search for `apify`.

1. Select the **apify** plugin and press `i` to install it.
...
1. Select the Apify MCP server to start the OAuth flow. Grok Build opens a browser tab for Apify.

1. Review the permissions and click **Allow access**. Apify requests access to your profile and to run Actors and manage your runs and storage on your behalf.

1. After you finish the OAuth flow, the **[needs auth]** label disappears and the Apify MCP server is connected.
...
If you're running Grok Build in a headless environment (SSH, remote container) or the OAuth flow still fails, authenticate with an API token instead. Copy your token from [Apify Console > Settings > Integrations](https://console.apify.com/settings/integrations) and configure the Apify MCP server with an `Authorization` header set to `Bearer <YOUR_API_TOKEN>`. See the [Apify MCP server documentation](/integrations/mcp) for details.

## Limitations

- Long-running Actors may exceed the time a single tool call waits for completion. Reduce the scope or split the work across multiple prompts.
- Each Actor run consumes Apify platform usage from your plan in addition to any Grok usage. See [Billing](/account/billing) for details.

## Related integrations

- [Claude Code CLI integration](/integrations/claude-code-cli) - Install the Apify plugin for Claude Code
