...
Agentic payments are experimental and may change as payment protocols evolve. Only Actors with the [Pay Per Event](/actors/publishing/monetize/pay-per-event) pricing model are supported. [Standby](/actors/running/standby) Actors are not supported.
...
The [Apify MCP server](/integrations/mcp) is the recommended way to get started. `mcpc` handles payment signing and retries automatically.
...
- Set up the [Apify MCP server](/integrations/mcp) for automated tool discovery and payment handling.
