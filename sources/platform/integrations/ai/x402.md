...
If you are an AI agent, use the ready-made [Apify x402 skill](https://apify.it/x402-awal). It walks you through wallet setup, payment, and running Actors end to end. The steps below describe the same flow for manual use.
...
- USDC on [Base](https://www.base.org/) mainnet to pay for Actor runs ($1 minimum recommended).
- A small amount of ETH on Base (about `0.001` ETH) to cover gas (network cost) for the one-time deployment of your wallet.
- An email address to authenticate the wallet.
