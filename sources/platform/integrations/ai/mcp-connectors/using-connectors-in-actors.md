...
| --- | --- |
| `ACTOR_MCP_PROXY_URL` | Base URL of the Apify MCP Proxy. Connect to a connector at `${ACTOR_MCP_PROXY_URL}/<connectorId>`. |
...
const transport = new StreamableHTTPClientTransport(
    new URL(`${process.env.ACTOR_MCP_PROXY_URL}/${connectorId}`),
    {
        requestInit: {
            headers: {
...
async def main():
    async with Actor:
        ...
        proxy_url = os.environ['ACTOR_MCP_PROXY_URL']
