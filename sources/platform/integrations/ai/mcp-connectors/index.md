...
flowchart LR
    subgraph actor [Your Actor]
        client["MCP client<br/><br/>Reads env vars:<br/>ACTOR_MCP_PROXY_URL<br/>APIFY_TOKEN"]
    end
...
1. When the Actor sends an MCP request to `ACTOR_MCP_PROXY_URL/<connectorId>`, the Apify MCP Proxy validates the request, injects your credentials, and forwards it to the upstream MCP server the connector is authorized against.
