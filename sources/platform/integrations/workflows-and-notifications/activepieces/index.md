...
When enabled, the trigger creates a webhook in your Apify account that sends events to Activepieces whenever the selected Actor or task run finishes with specified statuses.
...
1. Choose an action:
   ...
   - **Run task**: Starts an Apify task run
...
- **Input** (JSON overrides of the Actor's default input)
...
- **Watch Actor Runs**
  Triggers a flow on Apify Actor run events. You can:
...
- **Watch Task Runs**
  Triggers a flow on Apify task run events. You can:
...
- **Run Actor**
  Starts a specified Actor with customizable parameters:
...
- **Run task**
  Executes a specified Apify task with:
  - **Task**
  - **Input** JSON overrides
  - **Build**, **Memory**, **Timeout**
...
- **Get Dataset Items**
  Retrieves items from an Apify [dataset](/platform/storage/dataset):
...
- **Scrape Single URL**
  Runs a Website Content Crawler Actor for a specified website and returns its content:
  - Accepts a **URL** to scrape.
  - Lets you choose a **crawler type** (e.g., Cheerio, JSDOM, Playwright variants).
  - Returns the Actor run details and the scraped items (HTML, markdown, and structured data when available).

- **Get Key-Value Store Record**
  Retrieves a value from an Apify [key-value store](/platform/storage/key-value-store):
