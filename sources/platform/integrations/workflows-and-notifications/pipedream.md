...
- **Get key-value store record** - Retrieves a record from a [key-value store](/actors/storage/key-value-store).
- **Set key-value store record** - Creates or updates a record in a [key-value store](/actors/storage/key-value-store).
