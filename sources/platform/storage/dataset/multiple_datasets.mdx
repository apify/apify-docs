...
- Dataset aliases must match `[A-Za-z][A-Za-z0-9_]{0,100}`. Start each alias with a letter and use only letters, numbers, and underscores. Hyphens aren't supported.
...
See the full [Actor schema reference](/actors/development/actor-definition/actor-json#reference).
...
See [Datasets](/storage/dataset) for formats and query parameters.

## Surface datasets on the run page

The Storage tab shows data but doesn't surface it clearly to end users. To present datasets more prominently on the run page, define an [output schema](/actors/development/actor-definition/output-schema) that references each dataset by alias:
...
[Read more](/actors/development/actor-definition/output-schema#how-templates-work) about how templates work.
...
To charge for items in other datasets, implement custom billing in your Actor code. Refer to the [billing documentation](/actors/publishing/monetize/pay-per-event) for implementation details.
