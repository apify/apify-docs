...
import { ActorTemplateCard, theme } from '@apify/ui-library';
...
const ACTOR_TEMPLATES_GITHUB_MANIFEST_URL =
    ...
    if (isError) {
        return (
            <div>
                ...
                You can see them directly on{' '}
                <a href={ACTOR_TEMPLATES_GITHUB_URL} target="_blank" rel="noreferrer noopener">
                    GitHub
                </a>
                .
            </div>
    ...
    return (
        <ActorTemplatesWrapper>
            {actorTemplatesData?.map((actorTemplate: ActorTemplate) => {
                const { id, label, description, category, technologies } = actorTemplate;
