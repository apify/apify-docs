...
export default function GitButton({ href, ariaLabel }: GitButtonProps) {
    ...
    return (
        <StyledGitButton href={href} target="_blank" rel="noopener noreferrer" aria-label={ariaLabel}>
            <span className="GitButton__label">
                <GitHubMarkIcon />
                Star
            </span>
            {stars !== null && <span className="GitButton__count">{stars.toLocaleString('en-US')}</span>}
        </StyledGitButton>
    );
}
