...
`MCP connectors` is an Apify feature name. Use lowercase `connectors` when referring to the feature, and lowercase `connector` on its own when it refers to a specific connector created through the feature.
