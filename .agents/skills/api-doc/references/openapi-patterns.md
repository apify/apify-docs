...
Uniqueness beats the singular rule, because the operation ID becomes the page slug. `/actors/{actorId}/runs` can't be `actor_runs_post` - the account-wide `/actor-runs` family owns that slug and the two are different scopes. So Actor-scoped `/runs` and `/builds` use plural `actors_`, while `runs/last` has no clash and stays singular. Matching redirects: `nginx.conf`.

Never rewrite the `act_*` fragments in `x-legacy-doc-urls` or in-description links pointing at them. They're backward-compatibility anchors, not operation ID references - nginx can't see URL fragments, so they're the only thing keeping old anchor links alive.
