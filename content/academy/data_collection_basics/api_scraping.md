...
# [](#locating) Locating API endpoints
...
Let's say we want to receive all of the user's tracks in one request. Based on our observations of the endpoint's different parameters, we can modify the URL and utilize the "`limit`" option to return more than just twenty songs:

```
https://api-v2.soundcloud.com/users/141707/tracks?client_id=zdUqm51WRIAByd0lVLntcaWRKzuEIB4X&limit=99999
```

By using the ridiculously large number of `99999`, we ensure that all of the user's tracks will be captured in this single request.
...
const setCookieParser = require('set-cookie-parser'); // import the set-cookie-parser module

const getCookie = async () => {
    const response = await axios.get('https://www.zillow.com/'); // make a request to the target site

    const cookies = setCookieParser.parse(response); // parse the cookies from the response

    const cookieString = cookies.map(({ name, value }) => `${name}=${value};`).join(' '); // format the parsed data into a usable string

    console.log(cookieString); // log the final cookie string to be used in a 'cookie' header
};
...
const scrapeClientId = async () => {
    ...
    let clientId = null; // initialize a variable that will eventually hold the client_id

    page.on('response', async (res) => { // handle each response
        const id = new URL(res._url).searchParams.get('client_id') ?? null; // try to grab the 'client_id' parameter from each URL

        if (id) clientId = id; // if the parameter exists, set our clientId variable to the newly parsed value
    });

    await page.goto('https://soundcloud.com/tiesto/tracks'); // visit the page
    await page.waitForSelector('.sc-classic'); // wait for a selector that ensures the page has time to load and make requests to its API
